...
void USBD_AUDIO_SPKR_Sync(USBD_HandleTypeDef *pdev, AUDIO_SPKR_OffsetTypeDef offset)
{
  ...
  {
    ((USBD_AUDIO_SPKR_ItfTypeDef *)pdev->pUserData_UAC_SPKR)->AudioCmd(&haudio->buffer[0], BufferSize, AUDIO_SPKR_CMD_PLAY);
...
static uint8_t USBD_AUDIO_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  if (epnum == AUDIO_OUT_EP)
  {
    ...
    if (haudio->wr_ptr == AUDIO_TOTAL_BUF_SIZE)
    {
      ...
      {
        ((USBD_AUDIO_SPKR_ItfTypeDef *)pdev->pUserData_UAC_SPKR)->AudioCmd(&haudio->buffer[0], AUDIO_TOTAL_BUF_SIZE / 2U, AUDIO_SPKR_CMD_START);
        haudio->offset = AUDIO_SPKR_OFFSET_NONE;
      }
    }
